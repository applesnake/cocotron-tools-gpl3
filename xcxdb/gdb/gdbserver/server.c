...
handle_query (char *own_buf, int packet_len, int *new_packet_len_p)
{
  ...
  if (strncmp ("qXfer:libraries:read:", own_buf, 21) == 0)
    {
      CORE_ADDR ofs;
      unsigned int i, len, total_len;
      char *document, *p, *q, *r;
      struct inferior_list_entry *dll_ptr;
      char *annex;

      char cwdname[MAXPATHLEN + 1];
      getcwd(cwdname, MAXPATHLEN);
      ...
      for (dll_ptr = all_dlls.head; dll_ptr != NULL; dll_ptr = dll_ptr->next)
        {
          ...
          {
            if (*q == path_delimiter)
              *q = '\v';   // replace target system path delimiters by canonical delimiters
            q++;
          }

          strcpy (p, "  <library name=\"");
          p = p + strlen (p);
          name = xml_escape_text (r);
          strcpy (p, name);
          free (name);
          p = p + strlen (p);
