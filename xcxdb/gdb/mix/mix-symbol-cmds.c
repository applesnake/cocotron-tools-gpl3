...
enum mix_cmd_result
mix_cmd_symbol_list_lines (char *command, char **argv, int argc)
{
  char *filename;
  struct symtab *s;
  int i;
  ...
  if (argc != 1)
    error (_("mix_cmd_symbol_list_lines: Usage: SOURCE_FILENAME"));

  filename = argv[0];
  s = lookup_symtab (filename);

  if (s == NULL)
    error (_("mix_cmd_symbol_list_lines: Unknown source file name."));
