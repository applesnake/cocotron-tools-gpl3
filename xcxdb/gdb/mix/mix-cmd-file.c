...
enum mix_cmd_result
mix_cmd_file_list_exec_source_file (char *command, char **argv, int argc)
{
  struct symtab_and_line st;
  int optind = 0;
  char *optarg;
  
  if (!mix_valid_noargs ("mix_cmd_file_list_exec_source_file", argc, argv))
    error (_("mix_cmd_file_list_exec_source_file: Usage: No args"));
  ...
  if (!st.symtab)
    error (_("mix_cmd_file_list_exec_source_file: No symtab"));
...
enum mix_cmd_result
mix_cmd_file_list_exec_source_files (char *command, char **argv, int argc)
{
  struct symtab *s;
  struct partial_symtab *ps;
  struct objfile *objfile;

  if (!mix_valid_noargs ("mix_cmd_file_list_exec_source_files", argc, argv))
    error (_("mix_cmd_file_list_exec_source_files: Usage: No args"));
