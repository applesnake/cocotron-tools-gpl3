...
/* APPLE LOCAL: Sometimes although you try to run one thread, another
   thread will return anyway (for instance if it had a pending exception).
   In this case, we need to remind ourselves that we really intended
   to run the original thread.  Use SCHEDULER_LOCK_PTID for this purpose */
...
/* APPLE LOCAL: Record the current thread as the one you are trying to run.  */
void 
...
/* APPLE LOCAL: This returns whether the schedule lock is on or step.  */
int
scheduler_lock_on_p ()
{
  return ((scheduler_mode == schedlock_on) || (scheduler_mode == schedlock_step));
}
...
/* APPLE LOCAL: Set the scheduler locking mode to NEW_MODE.  Basically
   just translates between the enum and the strings used for the command. */
