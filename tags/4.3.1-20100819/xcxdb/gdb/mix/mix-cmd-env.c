...
extern void _initialize_mix_cmd_env (void);
...
enum mix_cmd_result
mix_cmd_env_pwd (char *command, char **argv, int argc)
{
  if (argc > 0)
    error (_("mix_cmd_env_pwd: No arguments required"));
          
  if (mix_version (uiout) < 2)
    {
...
enum mix_cmd_result
mix_cmd_env_cd (char *command, char **argv, int argc)
{
  if (argc == 0 || argc > 1)
    error (_("mix_cmd_env_cd: Usage DIRECTORY"));
...
enum mix_cmd_result
mix_cmd_env_path (char *command, char **argv, int argc)
{
  char *exec_path;
  char *env;
  int reset = 0;
  ...
  while (1)
    {
      int opt = mix_getopt ("mix_cmd_env_path", argc, argv, opts,
                           &optind, &optarg);
      if (opt < 0)
        break;
      switch ((enum opt) opt)
...
enum mix_cmd_result
mix_cmd_env_dir (char *command, char **argv, int argc)
{
  int i;
  int optind = 0;
  int reset = 0;
  ...
  while (1)
    {
      int opt = mix_getopt ("mix_cmd_env_dir", argc, argv, opts,
                           &optind, &optarg);
      if (opt < 0)
        break;
      switch ((enum opt) opt)
...
enum mix_cmd_result
mix_cmd_inferior_tty_set (char *command, char **argv, int argc)
{
...
enum mix_cmd_result
mix_cmd_inferior_tty_show (char *command, char **argv, int argc)
{
  const char *inferior_io_terminal = get_inferior_io_terminal ();
  
  if ( !mix_valid_noargs ("mix_cmd_inferior_tty_show", argc, argv))
    error (_("mix_cmd_inferior_tty_show: Usage: No args"));
