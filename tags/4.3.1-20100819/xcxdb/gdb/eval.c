/* Original in GNU gdb 6.8.50.20080826-cvs,
   modified on 2008-08-26 for xcxdb by Dr. Rolf Jansen. */
	...
	if (gnu_runtime)
	  {
	    struct type *type = selector_type;
            ...
            if (lookup_minimal_symbol ("_objc_checkObject", 0, 0))
            {
              argvec[0] = find_function_in_inferior ("_objc_checkObject", NULL);
              argvec[1] = target;
              argvec[2] = 0;
              ret = call_function_by_hand(argvec[0], 1, argvec + 1);
              if (value_as_long (ret) == 0)
                 error (_("Address does not point to an object."));
            }
