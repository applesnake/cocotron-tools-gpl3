...
static int mix_varobj_use_dynamic_type = 1;
/* APPLE LOCAL end */

/* APPLE LOCAL: We use this to lookup from fake child to type index.  */
static int mix_varobj_get_type_index_from_fake_child (struct varobj *parent, int index);
static int mix_varobj_value_struct_elt_by_index (struct varobj *parent, int index,
                                                 struct value **ret_val);
static struct type *mix_varobj_lookup_struct_elt_type_by_index (struct varobj *parent, int index);

/* Non-zero if we want to see trace of varobj level stuff.  */
int mix_varobjdebug = 0;
static void
mix_show_varobjdebug (struct ui_file *file, int from_tty,
...
static int mix_delete_variable (struct cpstack **, struct varobj *, int);

static void mix_delete_variable_1 (struct cpstack **, int *,
                                   struct varobj *, int, int);

static int mix_install_variable (struct varobj *);

static void mix_uninstall_variable (struct varobj *);

/* APPLE LOCAL */
static struct varobj *mix_child_exists (struct varobj *, int index);

static struct varobj *mix_create_child (struct varobj *, int, char *);

static void mix_save_child_in_parent (struct varobj *, struct varobj *);

static void mix_remove_child_from_parent (struct varobj *, struct varobj *);

/* Utility routines */

static struct varobj *mix_new_variable (void);

static struct varobj *mix_new_root_variable (void);

static void mix_free_variable (struct varobj *var);

static struct cleanup *mix_make_cleanup_free_variable (struct varobj *var);

static struct type *mix_get_type (struct varobj *var);

static struct type *mix_get_type_deref (struct varobj *var, int *was_ptr);

static struct type *mix_get_target_type (struct type *);

static enum varobj_display_formats mix_variable_default_display (struct varobj *);

static int mix_value_equal (struct value *, struct value *, int *);

static struct varobj_changelist *mix_varobj_changelist_init ();

static void mix_varobj_add_to_changelist(struct varobj_changelist *changelist, 
                                         struct varobj *var, 
                                         enum varobj_type_change type_changed);

static void mix_vpush (struct vstack **pstack, struct varobj *var);

static struct varobj *mix_vpop (struct vstack **pstack);

static void mix_cppush (struct cpstack **pstack, char *name);

static char *mix_cppop (struct cpstack **pstack);

/* Language-specific routines. */

static enum varobj_languages mix_variable_language (struct varobj *var);

static int mix_number_of_children (struct varobj *);

static char *mix_name_of_variable (struct varobj *);

static char *mix_path_expr_of_variable (struct varobj *);

static char *mix_make_name_of_child (struct varobj *, int);

static char *mix_path_expr_of_child (struct varobj *, int);

static struct value *mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *);

static struct value *mix_value_of_child (struct varobj *parent, int index,
                                         enum varobj_type_change *);

static struct type *mix_type_of_child (struct varobj *var);

static int mix_variable_editable (struct varobj *var);

static char *mix_value_of_variable (struct varobj *var);

/* APPLE LOCAL rename type_changeable */
static int mix_varobj_value_is_changeable_p (struct varobj *var);

static int is_root_var (struct varobj *var);

/* APPLE LOCAL set path expression junk.  */
static enum varobj_join_type mix_get_join_type (struct type *type);

/* C implementation */

static int c_mix_number_of_children (struct varobj *var);

static char *c_mix_make_name_of_child (struct varobj *parent, int index);

static char *c_mix_path_expr_of_child (struct varobj *parent, int index);

static struct value *c_mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *type_changed);

static struct value *c_mix_value_of_child (struct varobj *parent, int index, int *lookup_dynamic_type);

static struct type *c_mix_type_of_child (struct varobj *parent, int index);

static int c_mix_variable_editable (struct varobj *var);

static char *c_mix_value_of_variable (struct varobj *var);

/* C++ implementation */

static int cplus_mix_number_of_children (struct varobj *var);

static void cplus_mix_class_num_children (struct type *type, int children[3]);

static char *cplus_mix_make_name_of_child (struct varobj *parent, int index);

static char *cplus_mix_path_expr_of_child (struct varobj *parent, int index);

static struct value *cplus_mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *type_changed);

static struct value *cplus_mix_value_of_child (struct varobj *parent, int index, int *lookup_dynamic_type);

static struct type *cplus_mix_type_of_child (struct varobj *parent, int index);

static int cplus_mix_variable_editable (struct varobj *var);

static char *cplus_mix_value_of_variable (struct varobj *var);

/* Java implementation */

static int java_mix_number_of_children (struct varobj *var);

static char *java_mix_make_name_of_child (struct varobj *parent, int index);

static char *java_mix_path_expr_of_child (struct varobj *parent, int index);

static struct value *java_mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *type_changed);

static struct value *java_mix_value_of_child (struct varobj *parent, int index, int *lookup_dynamic_type);

static struct type *java_mix_type_of_child (struct varobj *parent, int index);

static int java_mix_variable_editable (struct varobj *var);

static char *java_mix_value_of_variable (struct varobj *var);
...
struct language_specific
{
  ...
  /* This makes & returns the name of the INDEX'th child of PARENT. */
...
static struct language_specific languages[vlang_end] =
{
  ...
  { vlang_unknown,
    c_mix_number_of_children,
    c_mix_make_name_of_child,
    c_mix_path_expr_of_child,
    c_mix_value_of_root,
    c_mix_value_of_child,
    c_mix_type_of_child,
    c_mix_variable_editable,
    c_mix_value_of_variable },
  ...
  /* C++ */
  { vlang_cplus,
    cplus_mix_number_of_children,
    cplus_mix_make_name_of_child,
    cplus_mix_path_expr_of_child,
    cplus_mix_value_of_root,
    cplus_mix_value_of_child,
    cplus_mix_type_of_child,
    cplus_mix_variable_editable,
    cplus_mix_value_of_variable },

  /* Java */
  { vlang_java,
    java_mix_number_of_children,
    java_mix_make_name_of_child,
    java_mix_path_expr_of_child,
    java_mix_value_of_root,
    java_mix_value_of_child,
    java_mix_type_of_child,
    java_mix_variable_editable,
    java_mix_value_of_variable }
};
...
static struct varobj_root *mix_rootlist;
/* Number of root varobjs in the list */
static int mix_rootcount = 0;

/* Prime number indicating the number of buckets in the hash table */
/* A prime large enough to avoid too many colisions */
#define VAROBJ_TABLE_SIZE 1999

/* Pointer to the varobj hash table (built at run time) */
static struct vlist **mix_varobj_table;
...
int mix_varobj_runs_all_threads = 0;
/* APPLE LOCAL end */
...
gdb_varobj_get_value (struct varobj *val1, char **result)
{
  ...
  TRY_CATCH (except, RETURN_MASK_ALL)
    {
      *result = mix_varobj_get_value (val1);
    }
...
mix_get_join_type (struct type *in_type)
{
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_PTR:
      {
        struct type *target = mix_get_target_type (type);
...
int
mix_evaluate_expression (struct expression *exp, struct value **value)
{
  ...
  volatile struct gdb_exception except;
  int old_unwind = unwind_on_signal_p;
  unwind_on_signal_p = 1;
...
/* APPLE LOCAL: New function. */
static int
mix_evaluate_type (struct expression *exp, struct value **value)
{
  /* APPLE LOCAL: The expression may contain a function, and we
     certainly don't want a crash in the function to leave the stack
     in a funny state. So set unwind-on-signal to clear up after any
...
static struct type *
mix_safe_value_rtti_target_type (struct value *val, int *full, int *top, int *using_enc)
{
  volatile struct gdb_exception except;
  struct ui_file *saved_gdb_stderr;
  static struct ui_file *null_stderr = NULL;
...
static struct value *
mix_varobj_fixup_value (struct value *in_value, 
  ...
  if (TYPE_CODE(base_type) == TYPE_CODE_PTR)
    {
      ...
      dynamic_type = mix_safe_value_rtti_target_type (in_value, &full, &top, &using_enc);
      if (dynamic_type)
        {
  ...
  else if (TYPE_CODE (base_type) == TYPE_CODE_REF)
    {
      ...
      target_type = mix_get_target_type (base_type);
      if (target_type != NULL)
        {
          int full, top, using_enc;
          
          deprecated_set_value_type (temp_val, lookup_pointer_type (target_type));
          dynamic_type = mix_safe_value_rtti_target_type (temp_val, &full, &top, &using_enc);
          if (dynamic_type)
...
mix_find_frame_addr_in_frame_chain (CORE_ADDR frame_addr)
{
...
struct varobj *
mix_varobj_create (char *objname,
                   ...
                   enum varobj_type type)
{
  ...
  var = mix_new_root_variable ();
  old_chain = mix_make_cleanup_free_variable (var);
  ...
  if (!mix_varobj_runs_all_threads)
  ...
  if (expression != NULL)
    {
      ...
      if ((type == USE_CURRENT_FRAME) || (type == USE_SELECTED_FRAME) || (type == USE_BLOCK_IN_FRAME))
        ...
        fi = mix_find_frame_addr_in_frame_chain (frame);
      ...
      var->format = mix_variable_default_display (var);
      ...
      if (var->root->exp != NULL)
        {
          ...
          if ((var->root->use_selected_frame || mix_varobj_pc_in_valid_block_p (var) || type == NO_FRAME_NEEDED) 
              && mix_evaluate_expression (var->root->exp, &var->value))
            {
              /* no error */

              var->root->in_scope = 1;
              var->type = value_type (var->value);
              var->value = mix_varobj_fixup_value (var->value, 
                                                   mix_varobj_use_dynamic_type, block,
                                                   &(var->dynamic_type),
          ...
          else
            {
              ...
              /* You might wonder how evaluate_type could get an error?
                 ...
                 contains a method call, we currently look up the function for that
                 implementation, and if the object is bad, the runtime can crash
                 in the lookup call...  */

              retval = mix_evaluate_type (var->root->exp, &var->value);
              if (retval != 0)
                {
                  var->type = value_type (var->value);
                  var->root->in_scope = 0;
          ...
          /* Set language info */
          lang = mix_variable_language (var);
  ...
  if (var != NULL && var->type != NULL)
      var->join_in_expr = mix_get_join_type (var->type);
...
/* Given an "objname", returns the pointer to the corresponding varobj or NULL if not found */

struct varobj *
mix_varobj_get_handle (char *objname)
{
  struct vlist *cv;
  const char *chp;
...
char *
mix_varobj_get_objname (struct varobj *var)
{
  return var->obj_name;
}
...
char *
mix_varobj_get_expression (struct varobj *var)
{
  return mix_name_of_variable (var);
}
...
int
mix_varobj_delete (struct varobj *var, char ***dellist, int only_children)
{
  int delcount;
  int mycount;
  ...
  mix_cppush (&result, NULL);

  if (only_children)
    /* Delete only the variable children */
    delcount = mix_delete_variable (&result, var, 1 /* only the children */ );
  else
    /* Delete the variable and all its children */
    delcount = mix_delete_variable (&result, var, 0 /* parent+children */ );
  ...
  if (dellist != NULL)
    {
      ...
      *cp = mix_cppop (&result);
      ...
      if (mycount || (*cp != NULL))
        warning (_("mix_varobj_delete: assertion failed - mycount(=%d) <> 0"), mycount);
    }
...
mix_varobj_set_display_format (struct varobj *var,
                               enum varobj_display_formats format)
{
  switch (format)
    {
...
enum varobj_display_formats
mix_varobj_get_display_format (struct varobj *var)
{
  return var->format;
}

int
mix_varobj_get_num_children (struct varobj *var)
{
  if (var->root->exp == NULL)
    return -1;

  if (var->num_children == -1)
    var->num_children = mix_number_of_children (var);
...
int
mix_varobj_list_children (struct varobj *var, struct varobj ***childlist)
{
  struct varobj *child;
  char *name;
  int i;
  ...
  for (i = 0; i < var->num_children; i++)
    {
      ...
      *((*childlist) + i) = NULL;
      ...
      child = mix_child_exists (var, i);
      if (child == NULL)
        {
          name = mix_make_name_of_child (var, i);
          child = mix_create_child (var, i, name);
        }
...
int 
mix_varobj_is_fake_child (struct varobj *var)
{
...
char *
mix_varobj_get_type (struct varobj *var)
{
  struct value *val;
...
char *
mix_varobj_get_dynamic_type (struct varobj *var)
{
  struct value *val;
...
struct type *
mix_varobj_get_type_struct (struct varobj *var)
{
  return mix_get_type (var);
}

char *
mix_varobj_get_path_expr (struct varobj *var)
{
  return mix_path_expr_of_variable (var);
}

/* Obtain the type of an object variable.  */

struct type *
mix_varobj_get_gdb_type (struct varobj *var)
{
  return var->type;
}

enum varobj_languages
mix_varobj_get_language (struct varobj *var)
{
  return mix_variable_language (var);
}
...
int
mix_varobj_in_scope_p (struct varobj *var)
{
  return var->root->in_scope;
}

int
mix_varobj_get_attributes (struct varobj *var)
{
  int attributes = 0;

  if (mix_variable_editable (var))
...
void 
mix_varobj_get_valid_block (struct varobj *var, CORE_ADDR *start, CORE_ADDR *end)
{
...
char *
mix_varobj_get_value (struct varobj *var)
{
  if (var->root->exp == NULL)
    return NULL;
    ...
    return mix_value_of_variable (var);
}
...
int
mix_varobj_set_value (struct varobj *var, char *expression)
{
  struct value *val;
  int error = 0;
  ...
  if (var->value != NULL && mix_variable_editable (var) && !var->error)
    {
      char *s = expression;
      ...
      if (!mix_evaluate_expression (exp, &value))
        {
      ...
      if (!mix_value_equal (var->value, value, &error))
        var->updated = 1;
...
int
mix_varobj_list (struct varobj ***varlist)
{
  struct varobj **cv;
  struct varobj_root *croot;
  int mycount = mix_rootcount;

  /* Alloc (rootcount + 1) entries for the result */
  *varlist = xmalloc ((mix_rootcount + 1) * sizeof (struct varobj *));
  ...
  *cv = NULL;
  ...
  if (mycount || (croot != NULL))
    warning ("mix_varobj_list: assertion failed - wrong tally of root vars (%d:%d)", mix_rootcount, mycount);

  return mix_rootcount;
}

/* Update the values for a variable and its children.  This is a
   two-pronged attack.  First, re-parse the value for the root's
   expression to see if it's changed.  Then go all the way through
   its children, reconstructing them and noting if they've changed.
   Return value:
...
int
mix_varobj_update (struct varobj **varp, struct varobj_changelist **changelist)
{
  int changed = 0;
  ...
  new = mix_value_of_root (varp, &type_changed);
  ...
  result = mix_varobj_changelist_init ();
  ...
  if (type_changed != VAROBJ_TYPE_UNCHANGED)
    {
      mix_varobj_add_to_changelist (result, *varp, type_changed);
      changed++;
    }
  ...
  /* If values are not equal, note that it's changed.
     ...
     We don't want some types to be reported as "changed". */
           ...
           || (mix_varobj_value_is_changeable_p (*varp)
               && ((*varp)->updated || !mix_value_equal ((*varp)->value, new, &error2))))
    {
      mix_varobj_add_to_changelist (result, *varp, type_changed);
      (*varp)->updated = 0;
      changed++;
  ...
  while (v != NULL)
    {
      ...
      new = mix_value_of_child (v->parent, v->index, &child_type_changed);
      if ((child_type_changed != VAROBJ_TYPE_UNCHANGED)
          || came_in_scope
          || (mix_varobj_value_is_changeable_p (v) 
              && (v->updated || !mix_value_equal (v->value, new, &error2))))
        {
          /* Note that it's changed */
          mix_varobj_add_to_changelist (result, v, child_type_changed);
          v->updated = 0;
          changed++;
        }
      /* Its value is going to be updated to NEW.  */
...
static int
mix_delete_variable (struct cpstack **resultp, struct varobj *var,
                     int only_children_p)
{
  int delcount = 0;

  mix_delete_variable_1 (resultp, &delcount, var,
...
static void
mix_delete_variable_1 (struct cpstack **resultp, int *delcountp,
                       ...
                       int remove_from_parent_p)
{
  ...
  for (vc = var->children; vc != NULL; vc = next)
    {
      ...
      mix_delete_variable_1 (resultp, delcountp, vc->child, 0, only_children_p);
      next = vc->next;
      xfree (vc);
    }
  ...
  if (var->obj_name != NULL)
    {
      mix_cppush (resultp, xstrdup (var->obj_name));
  ...
  if ((remove_from_parent_p) && (var->parent != NULL))
    {
      mix_remove_child_from_parent (var->parent, var);
    }

  if (var->obj_name != NULL)
    mix_uninstall_variable (var);

  /* Free memory associated with this variable */
  mix_free_variable (var);
}

/* Install the given variable VAR with the object name VAR->OBJ_NAME. */
static int
mix_install_variable (struct varobj *var)
{
  struct vlist *cv;
  struct vlist *newvl;
  const char *chp;
  ...
  newvl->next = *(mix_varobj_table + index);
  newvl->var = var;
  *(mix_varobj_table + index) = newvl;

  /* If root, add varobj to root list */
  if (is_root_var (var))
    {
      /* Add to list of root variables */
      if (mix_rootlist == NULL)
        var->root->next = NULL;
      else
        var->root->next = mix_rootlist;
      mix_rootlist = var->root;
      mix_rootcount++;
    }
...
static void
mix_uninstall_variable (struct varobj *var)
{
  struct vlist *cv;
  struct vlist *prev;
  ...
  if (prev == NULL)
    *(mix_varobj_table + index) = cv->next;
  else
    prev->next = cv->next;
  ...
  if (is_root_var (var))
    {
      ...
      if (mix_rootlist == var->root)
        mix_rootlist = var->root->next;
      else
        {
          prer = NULL;
          cr = mix_rootlist;
...
/* APPLE LOCAL */
mix_child_exists (struct varobj *var, int index)
{
  struct varobj_child *vc;
...
static struct varobj *
mix_create_child (struct varobj *parent, int index, char *name)
{
  struct varobj *child;
  char *childs_name;
  enum varobj_type_change type_changed;
  ...
  child = mix_new_variable ();

  /* name is allocated by mix_make_name_of_child */
  child->name = name;
  child->index = index;
  child->parent = parent;
  ...
  if (mix_variable_language (parent) == vlang_cplus
      && name[0] == 'p'
  ...
  mix_install_variable (child);

  /* Save a pointer to this child in the parent */
  mix_save_child_in_parent (parent, child);

  /* Now get the type & value of the child. */
  child->type = mix_type_of_child (child);
  ...
  if (CPLUS_FAKE_CHILD (parent))
  ...
  else
    {
      if (TYPE_CODE (parent->type) == TYPE_CODE_PTR)
        {
          target = mix_get_target_type (parent->type);
        }
      else
        target = parent->type;
      ...
      if (target != NULL
        ...
        child->join_in_expr = mix_get_join_type (child->type);
    }

  child->value = mix_value_of_child (parent, index, &type_changed);
...
static void
mix_save_child_in_parent (struct varobj *parent, struct varobj *child)
{
  struct varobj_child *vc;
...
static void
mix_remove_child_from_parent (struct varobj *parent, struct varobj *child)
{
...
static struct varobj *
mix_new_root_variable (void)
{
  struct varobj *var = mix_new_variable ();
...
static void
mix_free_variable (struct varobj *var)
{
...
static void
do_mix_free_variable_cleanup (void *var)
{
  mix_free_variable (var);
}

static struct cleanup *
mix_make_cleanup_free_variable (struct varobj *var)
{
  return make_cleanup (do_mix_free_variable_cleanup, var);
}
...
static struct type *
mix_get_type (struct varobj *var)
{
  struct type *type;

  if (mix_varobj_use_dynamic_type && var->dynamic_type != NULL)
...
static struct type *
mix_get_type_deref (struct varobj *var, int *was_ptr)
{
  struct type *type;

  type = mix_get_type (var);

  if (type != NULL && (TYPE_CODE (type) == TYPE_CODE_PTR || TYPE_CODE (type) == TYPE_CODE_REF))
    {
      type = mix_get_target_type (type);
      if (was_ptr != NULL)
        *was_ptr = 1;
    }
...
static struct type *
mix_get_target_type (struct type *type)
{
  if (type != NULL)
    {
...
mix_variable_default_display (struct varobj *var)
{
...
static int
mix_value_equal (struct value *val1, struct value *volatile val2, int *error2)
{
  int r, err1, err2;
...
static void
mix_varobj_add_to_changelist (struct varobj_changelist *changelist, 
                              struct varobj *var, 
                              enum varobj_type_change type_changed)
{
...
struct varobj *
mix_varobj_changelist_pop (struct varobj_changelist *changelist, 
                           enum varobj_type_change *type_changed)
{
  struct varobj_changelist_elem *s;
  struct varobj *v;
...
static void
mix_vpush (struct vstack **pstack, struct varobj *var)
{
  struct vstack *s;
...
/* FIXME: The following should be generic for any pointer */
static void
mix_cppush (struct cpstack **pstack, char *name)
{
  struct cpstack *s;
...
/* FIXME: The following should be generic for any pointer */
static char *
mix_cppop (struct cpstack **pstack)
{
  struct cpstack *s;
  char *v;
...
mix_variable_language (struct varobj *var)
{
  enum varobj_languages lang;
...
/* Return the number of children for a given variable.
   ...
   display. */
static int
mix_number_of_children (struct varobj *var)
{
...
static char *
mix_name_of_variable (struct varobj *var)
{
  /* APPLE LOCAL */
  return var->name;
}
...
static char *
mix_path_expr_of_root (struct varobj *var)
{
  ...
  if (mix_varobj_use_dynamic_type != 0
      && var->dynamic_type != NULL
      && var->dynamic_type != var->type)
    {
      ...
      root_type = mix_get_type_deref (var, &root_is_ptr);
      if (root_is_ptr)
        {
          const int formatLen = strlen ("(('%s' *) (%s))") - 3;
          dynamic_expr = TYPE_NAME (root_type);
          if (dynamic_expr != NULL && (dynamic_expr_len = strlen (dynamic_expr)) > 0)
            {
              root_name_len = strlen (var->name);
              path_expr = (char *) xmalloc (formatLen + dynamic_expr_len + root_name_len);
              sprintf (path_expr, "(('%s' *) (%s))", dynamic_expr, var->name);
            }
        }
    }
...
static char *
mix_path_expr_of_variable (struct varobj *var)
{
  /* APPLE LOCAL begin cast varobj root to dynamic type, if appropriate.  */
  if (var->path_expr != NULL && (!is_root_var (var)))
    return var->path_expr;
  else if (is_root_var (var))
    return mix_path_expr_of_root (var);
  /* APPLE LOCAL end cast varobj root to dynamic type, if appropriate.  */
  else if (var->elide_in_expr)
    {
      if (CPLUS_FAKE_CHILD (var->parent))
        /* FIXME: Note we won't get here for now, since I don't set
           ...
           really should work...  */
        var->path_expr = xstrdup (mix_path_expr_of_variable (var->parent->parent));
      else
        var->path_expr = xstrdup (mix_path_expr_of_variable (var->parent));
      return var->path_expr;
    }
  else
    return mix_path_expr_of_child (var->parent, var->index);
}
/* APPLE LOCAL end */

/* What is the name of the INDEX'th child of VAR? Returns a malloc'd string. */
static char *
mix_make_name_of_child (struct varobj *var, int index)
{
...
/* What is the rooted expression of the INDEX'th child of VAR? Returns
   a malloc'd string. */
static char *
mix_path_expr_of_child (struct varobj *var, int index)
{
  return (*var->root->lang->path_expr_of_child) (var, index);
}

int
mix_varobj_type_is_equal_p (struct varobj *old_var, struct varobj *new_var)
{
  ...
  old_type = mix_varobj_get_type (old_var);
  new_type = mix_varobj_get_type (new_var);
...
static struct value *
/* APPLE LOCAL */
mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *type_changed)
{
  struct varobj *var;
  ...
  if (var->root->use_selected_frame || mix_get_type (var) == NULL)
    /* APPLE LOCAL end */
    {
      struct varobj *tmp_var;

      tmp_var = mix_varobj_create (NULL, mix_name_of_variable (var), (CORE_ADDR) 0, NULL, USE_SELECTED_FRAME);
      ...
      if (tmp_var == NULL)
        ...
        {
          mix_free_variable (tmp_var);
          return NULL;
        }
      if (mix_varobj_type_is_equal_p (tmp_var, var))
        {
          if ((var->root->valid_block != NULL 
          ...
          if (tmp_var->root->in_scope 
            ...
            {
              mix_varobj_delete (tmp_var, NULL, 0);
              *type_changed = VAROBJ_TYPE_UNCHANGED;
            }
        }
      else
        {
          ...
          mix_varobj_delete (var, NULL, 0);
          mix_install_variable (tmp_var);
          *var_handle = tmp_var;
          var = *var_handle;
          *type_changed = VAROBJ_TYPE_CHANGED;
        }
  ...
  else
    {
      *type_changed = VAROBJ_TYPE_UNCHANGED;
      ...
      if (!mix_varobj_pc_in_valid_block_p (var))
        return NULL;
...
int
mix_varobj_pc_in_valid_block_p (struct varobj *var)
{
  struct frame_info *fi;
  CORE_ADDR cur_pc;
  ...
  if (fi != NULL)
    {
      ...
      return (BLOCK_START (var->root->valid_block) <= cur_pc && cur_pc < BLOCK_END (var->root->valid_block));
    }
  else
    {
      return 0;
...
static struct value *
mix_value_of_child (struct varobj *parent, int index, 
                    enum varobj_type_change *type_changed)
{
  struct value *value;
  struct varobj *child;
  ...
  *type_changed = VAROBJ_TYPE_UNCHANGED;
  ...
  child = mix_child_exists (parent, index);

  if (child == NULL)
    error ("mix_value_of_child called with a NULL child");
  ...
  if (lookup_dynamic_type)
    {
      ...
      new_value = mix_varobj_fixup_value (value, mix_varobj_use_dynamic_type, 
      ...
      if (child->dynamic_type == NULL)
        {
          if (child->dynamic_type_name == NULL)
            ...
            {
              if (dynamic_type_name == NULL || strcmp (child->dynamic_type_name, dynamic_type_name) != 0)
                {
                  xfree (child->dynamic_type_name);
                  child->dynamic_type_name = dynamic_type_name;
                  *type_changed = VAROBJ_DYNAMIC_TYPE_CHANGED;
...
static struct type *
mix_type_of_child (struct varobj *var)
{
...
/* Is this variable editable? Use the variable's type to make
   this determination. */
static int
mix_variable_editable (struct varobj *var)
{
...
static char *
mix_value_of_variable (struct varobj *var)
{
...
static int
/* APPLE LOCAL rename type_changeable */
mix_varobj_value_is_changeable_p (struct varobj *var)
{
  int r;
  struct type *type;
...
/* C */
static int
c_mix_number_of_children (struct varobj *var)
{
  struct type *type;
  struct type *target;
  int children;
  ...
  target = mix_get_target_type (type);
  children = 0;
...
static char *
c_mix_make_name_of_child (struct varobj *parent, int index)
{
  struct type *type;
  struct type *target;
  char *name;
...
static char *
c_mix_path_expr_of_child (struct varobj *parent, int index)
{
  char *path_expr = NULL;
  struct varobj *child = mix_child_exists (parent, index);
  char *parent_expr;
  char *name;
  int parent_len, child_len, len;

  if (child == NULL)
    error ("c_mix_path_expr_of_child: Tried to get path expression for a null child.");

  parent_expr = mix_path_expr_of_variable (parent);
  name = mix_name_of_variable (child);
...
static struct value *
c_mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *type_changed)
{
  ...
  if (within_scope)
    {
      ...
      if (mix_evaluate_expression (var->root->exp, &new_val))
        {
          struct type *dynamic_type;
          char *dynamic_type_name;
          new_val = mix_varobj_fixup_value (new_val, mix_varobj_use_dynamic_type, 
                                            var->root->valid_block,
                                            &dynamic_type, &dynamic_type_name);
          if (mix_varobj_use_dynamic_type)
            {
              ...
              if (var->dynamic_type == NULL)
                {
                  if (var->dynamic_type_name == NULL)
                    ...
                    {
                      if (dynamic_type_name == NULL || strcmp (dynamic_type_name, var->dynamic_type_name) != 0)
                        {
                          *type_changed = VAROBJ_DYNAMIC_TYPE_CHANGED;
                          xfree (var->dynamic_type_name);
                          var->dynamic_type_name = dynamic_type_name;
...
static struct type *
mix_varobj_lookup_struct_elt_type_by_index (struct varobj *parent, int index)
{
  struct type *type;
  int type_index;
  ...
  if (CPLUS_FAKE_CHILD (parent))
    {
      type_index = mix_varobj_get_type_index_from_fake_child (parent, index);
      type = mix_get_type (parent->parent);
    }
  else
    {
      type = mix_get_type (parent);
...
/* APPLE LOCAL: varobj_value_struct_elt_by_index does the same
   ...
   Sets RET_VAL to the value, and returns 1 on success, 0 on failure.  */

static int
mix_varobj_value_struct_elt_by_index (struct varobj *parent, int index, struct value **ret_val)
{
  struct type *t;
  ...
  if (TYPE_CODE (t) == TYPE_CODE_MEMBERPTR)
    error (_("not implemented: member type in mix_varobj_value_struct_elt_by_index"));
...
static int
mix_varobj_get_type_index_from_fake_child (struct varobj *parent, int index)
{
  struct type* type;
  int type_index = -1;
  ...
  if (!CPLUS_FAKE_CHILD (parent))
  ...
  else
    {
      ...
      type = mix_get_type_deref (parent->parent, NULL);
    }
...
static struct value *
c_mix_value_of_child (struct varobj *parent, int index, int *lookup_dynamic_type)
{
  struct value *value;
  struct value *temp;
  struct value *indval;
  ...
  if (child == NULL)
    error ("c_mix_value_of_child: called with NULL child");
  ...
  if (temp != NULL)
    {
      switch (TYPE_CODE (type))
...
#else
          indval = value_from_longest (builtin_type_int32, (LONGEST) index);
        ...
        case TYPE_CODE_UNION:
          ...
          mix_varobj_value_struct_elt_by_index (parent, index, &value);
          /* END APPLE LOCAL */
          break;
...
static struct type *
c_mix_type_of_child (struct varobj *parent, int index)
{
  struct type *type;
  struct varobj *child;
  struct type *parent_type = mix_get_type (parent);
  struct type *target_type;
  ...
  child = mix_child_exists (parent, index);
  if (child == NULL)
    error ("c_mix_type_of_child: called with a NULL child.");
  ...
  switch (TYPE_CODE (parent_type))
    ...
    case TYPE_CODE_UNION:
      ...
      type = mix_varobj_lookup_struct_elt_type_by_index (parent, index);
      /* END APPLE LOCAL */
      break;
    ...
    case TYPE_CODE_PTR:
      /* Be careful here, this might be a pointer pointing to a typedef, 
         ...
         wrong. */
      target_type = check_typedef (mix_get_target_type (parent_type));
    ...
    default:
      ...
      error ("Child of parent: \"%s\" whose type: \"%d\" does not allow children",
             mix_name_of_variable (parent), TYPE_CODE (parent_type));
      break;
    }
...
static int
c_mix_variable_editable (struct varobj *var)
{
  switch (TYPE_CODE (mix_get_type (var)))
    {
    case TYPE_CODE_STRUCT:
    case TYPE_CODE_UNION:
    case TYPE_CODE_ARRAY:
...
static char *
c_mix_value_of_variable (struct varobj *var)
{
    ...
    case TYPE_CODE_ARRAY:
      {
        ...
        number = xstrprintf ("[%d]", mix_varobj_get_num_children (var));
        return (number);
      }
      /* break; */
        ...
        if (var->value == NULL)
        ...
        else
          {
            ...
            common_val_print (var->value, stb,
                              format_code[(int) var->format], 1, 0, 0, current_language);
...
static int
cplus_mix_number_of_children (struct varobj *var)
{
  struct type *type;
  ...
  if (!CPLUS_FAKE_CHILD (var))
    {
      type = mix_get_type_deref (var, NULL);
      ...
      if (type == NULL)
      ...
      else if (((TYPE_CODE (type)) == TYPE_CODE_STRUCT) || ((TYPE_CODE (type)) == TYPE_CODE_UNION))
        {
          ...
          cplus_mix_class_num_children (type, kids);
          if (kids[v_public] != 0)
            children++;
          if (kids[v_private] != 0)
            children++;
  ...
  else
    {
      ...
      type = mix_get_type_deref (var->parent, NULL);

      cplus_mix_class_num_children (type, kids);
      if (strcmp (mix_name_of_variable (var), "public") == 0)
        children = kids[v_public];
      else if (strcmp (mix_name_of_variable (var), "private") == 0)
  ...
  if (dont_know)
    children = c_mix_number_of_children (var);
...
static void
cplus_mix_class_num_children (struct type *type, int children[3])
{
  int i;
...
static int
cplus_mix_real_type_index_for_fake_child_index (struct type *type, 
                                                enum vsections prot, 
                                                int num)
{
  int num_found = 0;
...
static char *
cplus_mix_make_name_of_child (struct varobj *parent, int index)
{
  char *name;
  struct type *type;
  ...
  if (CPLUS_FAKE_CHILD (parent))
    ...
    type = mix_get_type_deref (parent, NULL);
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_UNION:
      if (CPLUS_FAKE_CHILD (parent))
        {
          int type_index = mix_varobj_get_type_index_from_fake_child (parent, index);
        ...
        {
          int children[3];
          cplus_mix_class_num_children(type, children);
  ...
  if (name == NULL)
    return c_mix_make_name_of_child (parent, index);
  else
    {
      if (name != NULL)
...
static char *
cplus_mix_path_expr_of_child (struct varobj *parent, int index)
{
  char *path_expr;
  struct type *type;
  int children[3];
  struct varobj *child = mix_child_exists (parent, index);
  char *parent_expr = mix_path_expr_of_variable (parent);
  ...
  if (child == NULL)
    error ("cplus_mix_path_expr_of_child: Tried to get path expression for a null child.");

  /* If the child has a NULL or empty name it must be an anonomyous
     structure or union.  In that case, return the parent's name.  */
  if (mix_name_of_variable (child) == NULL || strlen (mix_name_of_variable (child)) == 0)
    return parent_expr;
  ...
  if (CPLUS_FAKE_CHILD (parent))
    {
      /* Looking for children of public, private, or protected. */
      type = mix_get_type_deref (parent->parent, &is_ptr);
    }
  else
    type = mix_get_type_deref (parent, &is_ptr);
  ...
  if (TYPE_RUNTIME (type) == OBJC_RUNTIME)
    return c_mix_path_expr_of_child (parent, index);
      ...
      if (CPLUS_FAKE_CHILD (parent))
        {
          int index_in_type;
          enum vsections prot;
          char *parent_name = mix_name_of_variable (parent);
          int child_is_ptr;
          int dynamic_expr_len, join_expr_len;
          char *dynamic_expr = NULL;
          char *join_expr;
          ...
          if (strcmp (parent_name, "private") == 0)
            ...
            {
              error ("cplus_mix_make_name_of_child got a parent with invalid fake child name: \"%s\".", parent_name);
              return NULL;
            }

          index_in_type = cplus_mix_real_type_index_for_fake_child_index (type, prot, index);
          ...
          if (mix_varobj_use_dynamic_type != 0 
              ...
              && child->dynamic_type != child->type)
            {
              ...
              child_type = mix_get_type_deref (child, &child_is_ptr);
              if (!child_is_ptr)
                dynamic_expr_len = 0;
              else
                {
          ...
          if (dynamic_expr_len > 0)
                {
                  const int formatLen = strlen ("(('%s' *) ((%s)%s%s))") - 6 + 1;
                  path_expr = (char *) xmalloc (formatLen + dynamic_expr_len + parent_len + join_expr_len + child_len);
                  sprintf (path_expr, "(('%s' *) ((%s)%s%s))", dynamic_expr, parent_expr, join_expr, child_name);
                }
          else
            {
              const int formatLen = strlen ("((%s)%s%s)") - 4 + 1;
              path_expr = (char *) xmalloc (formatLen + parent_len + join_expr_len + child_len);
...
static struct value *
cplus_mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *type_changed)
{
  return c_mix_value_of_root (var_handle, type_changed);
}

static struct value *
cplus_mix_value_of_child (struct varobj *parent, int index, int *lookup_dynamic_type)
{
  struct type *type;
  struct value *value;
  int is_ptr;
      ...
      if (CPLUS_FAKE_CHILD (parent))
        {
          ...
          child = mix_child_exists (parent, index);
          if (!child)
            error ("cplus_mix_value_of_child: Tried to get the value of a null child.");
          /* APPLE LOCAL: Can't use the value_struct_elt, since that
             looks up by name which doesn't work for anonymous unions
             & structures.  */
          ret_val = mix_varobj_value_struct_elt_by_index (parent, index, &value);
          if (!ret_val)
            return NULL;
  ...
  if (value == NULL)
    return c_mix_value_of_child (parent, index, lookup_dynamic_type);

  return value;
}

static struct type *
cplus_mix_type_of_child (struct varobj *parent, int index)
{
  struct type *type, *t;
  int is_ptr; 

  if (CPLUS_FAKE_CHILD (parent))
    t = mix_get_type_deref (parent->parent, &is_ptr);
  else
    t = mix_get_type_deref (parent, &is_ptr);
  ...
  if (type == NULL)
    return c_mix_type_of_child (parent, index);

  return type;
}

static int
cplus_mix_variable_editable (struct varobj *var)
{
  if (CPLUS_FAKE_CHILD (var))
    return 0;

  return c_mix_variable_editable (var);
}

static char *
cplus_mix_value_of_variable (struct varobj *var)
{
  ...
  switch (TYPE_CODE (mix_get_type (var)))
    ...
    case TYPE_CODE_REF:
      {
        ...
        int was_ptr;
        real_type = mix_get_type_deref (var, &was_ptr);
        if (real_type 
    ...
    default:
      return c_mix_value_of_variable (var);
    }
}
...
static int
java_mix_number_of_children (struct varobj *var)
{
  return cplus_mix_number_of_children (var);
}

static char *
java_mix_make_name_of_child (struct varobj *parent, int index)
{
  char *name, *p;

  name = cplus_mix_make_name_of_child (parent, index);
...
static struct value *
java_mix_value_of_root (struct varobj **var_handle, enum varobj_type_change *type_changed)
{
  return cplus_mix_value_of_root (var_handle, type_changed);
}

static struct value *
java_mix_value_of_child (struct varobj *parent, int index, int *lookup_dynamic_type)
{
  return cplus_mix_value_of_child (parent, index, lookup_dynamic_type);
}

static struct type *
java_mix_type_of_child (struct varobj *parent, int index)
{
  return cplus_mix_type_of_child (parent, index);
}

static int
java_mix_variable_editable (struct varobj *var)
{
  return cplus_mix_variable_editable (var);
}

static char *
java_mix_value_of_variable (struct varobj *var)
{
  return cplus_mix_value_of_variable (var);
}
static char *
java_mix_path_expr_of_child (struct varobj *parent, int index)
{
  return cplus_mix_path_expr_of_child (parent, index);
}

extern void _initialize_varobj_mix (void);
void
_initialize_varobj_mix (void)
{
  int sizeof_table = sizeof (struct vlist *) * VAROBJ_TABLE_SIZE;

  mix_varobj_table = xmalloc (sizeof_table);
  memset (mix_varobj_table, 0, sizeof_table);

  /* APPLE LOCAL begin varobj */
  add_setshow_boolean_cmd ("varobj-print-object", class_obscure,
                           &mix_varobj_use_dynamic_type, _("\
