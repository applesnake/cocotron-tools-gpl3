...
/* The arguments to be passed on the command line and parsed here are:
   ...
   respectively. */
enum mix_cmd_result
mix_cmd_disassemble (char *command, char **argv, int argc)
{
  enum mix_cmd_result retval;
  CORE_ADDR start;
  ...
  while (1)
    {
      int opt = mix_getopt ("mix_cmd_disassemble", argc, argv, opts,
                           &optind, &optarg);
      if (opt < 0)
        break;
      switch ((enum opt) opt)
  ...
  if (!((line_seen && file_seen && num_seen && !start_seen && !end_seen)
        ...
        || (!line_seen && !file_seen && !num_seen && start_seen && end_seen)))
    error
      ("mix_cmd_disassemble: Usage: ( [-f filename -l linenum [-n howmany]] | [-s startaddr -e endaddr]) [--] mixed_mode.");
  ...
  if ((mixed_source_and_assembly != 0) && (mixed_source_and_assembly != 1))
    error (_("mix_cmd_disassemble: Mixed_mode argument must be 0 or 1."));
  ...
  if (line_seen && file_seen)
    {
      ...
      if (s == NULL)
        error (_("mix_cmd_disassemble: Invalid filename."));
      if (!find_line_pc (s, line_num, &start))
        error (_("mix_cmd_disassemble: Invalid line number"));
      if (find_pc_partial_function (start, NULL, &low, &high) == 0)
        error (_("mix_cmd_disassemble: No function contains specified address"));
    }
