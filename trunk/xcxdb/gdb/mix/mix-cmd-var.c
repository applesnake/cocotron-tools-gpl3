...
const char mix_no_values[] = "--no-values";
const char mix_simple_values[] = "--simple-values";
const char mix_make_varobjs[] = "--make-varobjs";
const char mix_all_values[] = "--all-values";

extern int varobjdebug;         /* defined in varobj.c.  */

/* This is a useful function for reporting the creation of a varobj in a standard way.  */
void mix_report_var_creation (struct ui_out *uiout,
                              struct varobj *var,
                              int is_root);

static char *typecode_as_string (struct varobj* var);

static struct ui_out *mix_tmp_out = NULL;

static void mix_prepare_tmp_out (void);

static int mix_show_protections = 1;

static int mix_varobj_update_one (struct varobj *var,
                                  enum print_values print_values);

static int mix_print_value_p (struct type *type, enum print_values print_values);


/* VAROBJ operations */

enum mix_cmd_result
mix_cmd_var_create (char *command, char **argv, int argc)
{
  ...
  struct cleanup *mix_out_cleanup;
  enum varobj_type var_type = USE_SELECTED_FRAME;

  if (argc != 3)
    error (_("mix_cmd_var_create: Usage: NAME FRAME EXPRESSION."));
  ...
  if (strcmp (name, "-") == 0)
    {
      xfree (name);
      name = mix_varobj_gen_name ();
    }
  else if (!isalpha (*name))
    error (_("mix_cmd_var_create: name of object must begin with a letter"));
  ...
  if (strcmp (frame, "*") == 0)
  ...
  else if (frame[0] == '+')
    {
      /* The '+' indicates the variable is to be created by associating it with 
         a block which can be done by address (+0x11223300) or by file:line (+foo.c:12).  */
      ...
      if (strspn (block_addr, "0x123456789abcdefABCDEF") == strlen (block_addr))
        {
          ...
          if (except.reason < 0)
            {
              error ("mix_cmd_var_create: invalid address in block expression: \"%s\"", frame);
            }
        }
      else
        {
          ...
          if (colon)
              ...
              TRY_CATCH (except, RETURN_MASK_ALL)
                {
                  ...
                  char *s = block_addr; 
                  sals = decode_line_1 (&s, 1, (struct symtab *) NULL, 0, NULL, NULL);
                }
              ...
              if (except.reason >= 0 && sals.nelts >= 1)
                {
                  ...
                  block = BLOCKVECTOR_BLOCK (BLOCKVECTOR (sals.sals[0].symtab), GLOBAL_BLOCK);
                ...
                {
                  error ("mix_cmd_var_create: invalid file and line in block expression: \"%s\"", frame);
                }
            ...
            {
              error ("mix_cmd_var_create: missing ':' in file and line block expression: \"%s\"", frame);
            }
        }
    }
  else
  ...
  if (varobjdebug)
    fprintf_unfiltered (gdb_stdlog, "Name=\"%s\", Frame=\"%s\" (0x%s), Expression=\"%s\"\n",
                        name, frame, paddr (frameaddr), expr);
  
  mix_prepare_tmp_out ();
  mix_out_cleanup = make_cleanup_restore_uiout (uiout);
  uiout = mix_tmp_out;
  var = mix_varobj_create (name, expr, frameaddr, block, var_type);
  do_cleanups (mix_out_cleanup);

  if (var == NULL)
    error ("mix_cmd_var_create: unable to create variable object");
  
  mix_report_var_creation (uiout, var, 1);
...
void
mix_report_var_creation (struct ui_out *uiout, struct varobj *var, int is_root)
{  
  char *type;
  ...
  ui_out_field_string (uiout, "name", mix_varobj_get_objname (var));

  /* For child variables, we print out the expression.  Put it here because
     that's how it is in the testsuite! */
  if (!is_root)
    ui_out_field_string (uiout, "exp", mix_varobj_get_expression (var));

  ui_out_field_int (uiout, "numchild", mix_varobj_get_num_children (var));
  type = mix_varobj_get_type (var);

  if (type == NULL)
    {
      if (mix_varobj_is_fake_child (var))
        {
  ...
  type = mix_varobj_get_dynamic_type (var);
  if (type == NULL)
    {
  ...
  if (is_root)
    {
      ...
      if (mix_varobj_in_scope_p (var))
        ui_out_field_string (uiout, "in_scope", "true");
      else
        ui_out_field_string (uiout, "in_scope", "false");

      mix_varobj_get_valid_block (var, &block_start, &block_end);
...
typecode_as_string (struct varobj *var)
{
  ...
  type = mix_varobj_get_type_struct (var);
  if (type == NULL)
...
enum mix_cmd_result
mix_cmd_var_delete (char *command, char **argv, int argc)
{
  char *name;
  char *expr;
  struct varobj *var;
  ...
  if (argc < 1 || argc > 2)
    error (_("mix_cmd_var_delete: Usage: [-c] EXPRESSION."));

  name = xstrdup (argv[0]);
  /* Add cleanup for name. Must be free_current_contents as name can be reallocated */
  old_cleanups = make_cleanup (free_current_contents, &name);

  /* If we have one single argument it cannot be '-c' or any string starting with '-'. */
  if (argc == 1)
    {
      if (strcmp (name, "-c") == 0)
        error (_("mix_cmd_var_delete: Missing required argument after '-c': variable object name"));
      if (*name == '-')
        error (_("mix_cmd_var_delete: Illegal variable object name"));
    }
  ...
  if (argc == 2)
    {
      expr = xstrdup (argv[1]);
      if (strcmp (name, "-c") != 0)
        error (_("mix_cmd_var_delete: Invalid option."));
      children_only_p = 1;
      xfree (name);
      name = xstrdup (expr);
      xfree (expr);
  ...
  /* If we didn't error out, now NAME contains the name of the variable. */

  var = mix_varobj_get_handle (name);

  if (var == NULL)
    error (_("mix_cmd_var_delete: Variable object not found."));

  numdel = mix_varobj_delete (var, NULL, children_only_p);
...
enum mix_cmd_result
mix_cmd_var_set_format (char *command, char **argv, int argc)
{
  ...
  if (argc != 2)
    error (_("mix_cmd_var_set_format: Usage: NAME FORMAT."));

  /* Get varobj handle, if a valid var obj name was specified */
  var = mix_varobj_get_handle (argv[0]);

  if (var == NULL)
    error (_("mix_cmd_var_set_format: Variable object not found"));

  formspec = xstrdup (argv[1]);
  if (formspec == NULL)
    error (_("mix_cmd_var_set_format: Must specify the format as: \"natural\", \"binary\", \"decimal\", \"hexadecimal\", \"unsigned\", or \"octal\""));
  ...
  if (strncmp (formspec, "natural", len) == 0)
    ...
    error (_("mix_cmd_var_set_format: Unknown display format: must be: \"natural\", \"binary\", \"decimal\", \"hexadecimal\",  \"unsigned\", or \"octal\""));

  /* Set the format of VAR to given format */
  mix_varobj_set_display_format (var, format);

  /* Report the new current format */
  ui_out_field_string (uiout, "format", mix_varobj_format_string[(int) format]);
  return MIX_CMD_DONE;
}

enum mix_cmd_result
mix_cmd_var_show_format (char *command, char **argv, int argc)
{
  enum varobj_display_formats format;
  struct varobj *var;

  if (argc != 1)
    error (_("mix_cmd_var_show_format: Usage: NAME."));

  /* Get varobj handle, if a valid var obj name was specified */
  var = mix_varobj_get_handle (argv[0]);
  if (var == NULL)
    error (_("mix_cmd_var_show_format: Variable object not found"));

  format = mix_varobj_get_display_format (var);
...
enum mix_cmd_result
mix_cmd_var_info_num_children (char *command, char **argv, int argc)
{
  struct varobj *var;

  if (argc != 1)
    error (_("mix_cmd_var_info_num_children: Usage: NAME."));

  /* Get varobj handle, if a valid var obj name was specified */
  var = mix_varobj_get_handle (argv[0]);
  if (var == NULL)
    error (_("mix_cmd_var_info_num_children: Variable object not found"));
...
mix_parse_values_option (const char *arg)
{
  if (strcmp (arg, "0") == 0
      || strcmp (arg, mix_no_values) == 0)
    return PRINT_NO_VALUES;
  else if (strcmp (arg, "1") == 0
           || strcmp (arg, mix_all_values) == 0)
    return PRINT_ALL_VALUES;
  else if (strcmp (arg, "2") == 0
           || strcmp (arg, mix_simple_values) == 0)
    return PRINT_SIMPLE_VALUES;
  else
    error (_("Unknown value for PRINT_VALUES\n\
Must be: 0 or \"%s\", 1 or \"%s\", 2 or \"%s\""),
           mix_no_values, mix_simple_values, mix_all_values);
}

/* Return 1 if given the argument PRINT_VALUES we should display a value of type TYPE.  */

static int
mix_print_value_p (struct type *type, enum print_values print_values)
{
  if (type != NULL)
...
enum mix_cmd_result
mix_cmd_var_list_children (char *command, char **argv, int argc)
{
  ...
  int argv0_is_flag = 0;
  int argv1_is_flag = 0;
  int mix_show_fake_children = mix_show_protections;
  int saw_fake_child, saw_public, saw_other;
  int num_fake_childs_children;

  const char *usage = "mix_cmd_var_list_children: Usage: [--suppress-protection|--show-protection] "
  ...
  if (strcmp (argv[0], "--suppress-protection") == 0)
    {
      mix_show_fake_children = 0;
      argv++;
      argc--;
    }
  else if (strcmp (argv[0], "--show-protection") == 0)
    {
      mix_show_fake_children = 1;
      argv++;
      argc--;
    }
  ...
  if (argc == 1 && !argv0_is_flag)
    ...
    var = mix_varobj_get_handle (argv[1]);
  ...
  numchild = mix_varobj_list_children (var, &childlist);
  ...
  while (*cc != NULL)
    {
      if (mix_varobj_is_fake_child (*cc))
        {
          if (strcmp (mix_varobj_get_expression (*cc), "public") == 0)
            {
              saw_public = 1;
            }
          else
          ...
          num_fake_childs_children += mix_varobj_get_num_children (*cc) - 1;
          saw_fake_child = 1;
        } 
      cc++;
    }
  ...
  while (*cc != NULL)
    {
      ...
      if (mix_varobj_is_fake_child (*cc) && !mix_show_fake_children) 
        {
          struct varobj **fake_childlist, **cc2;
          int num_fake;
          num_fake = mix_varobj_list_children (*cc, &fake_childlist);
              ...
              while (*cc2 != NULL) 
                {
                  ...
                  mix_report_var_creation (uiout, *cc2, 0);
                  
                  if (print_values)
                    ui_out_field_string (uiout, "value", mix_varobj_get_value (*cc2));
      ...
      else
        {
          ...
          mix_report_var_creation (uiout, *cc, 0);
          
          if (print_values)
            ui_out_field_string (uiout, "value", mix_varobj_get_value (*cc));
          do_cleanups (cleanup_child);
        }
      cc++;
    }
...
enum mix_cmd_result
mix_cmd_var_info_type (char *command, char **argv, int argc)
{
  struct varobj *var;

  if (argc != 1)
    error (_("mix_cmd_var_info_type: Usage: NAME."));

  /* Get varobj handle, if a valid var obj name was specified */
  var = mix_varobj_get_handle (argv[0]);
  if (var == NULL)
    error (_("mix_cmd_var_info_type: Variable object not found"));

  ui_out_field_string (uiout, "type", mix_varobj_get_type (var));
  return MIX_CMD_DONE;
}

enum mix_cmd_result
mix_cmd_var_info_path_expression (char *command, char **argv, int argc)
{
  struct varobj *var;
  char *path_expr;
  ...
  path_expr = mix_varobj_get_path_expr (var);
...
enum mix_cmd_result
mix_cmd_var_info_expression (char *command, char **argv, int argc)
{
  enum varobj_languages lang;
  struct varobj *var;

  if (argc != 1)
    error (_("mix_cmd_var_info_expression: Usage: NAME."));

  /* Get varobj handle, if a valid var obj name was specified */
  var = mix_varobj_get_handle (argv[0]);
  if (var == NULL)
    error (_("mix_cmd_var_info_expression: Variable object not found"));

  lang = mix_varobj_get_language (var);
...
enum mix_cmd_result
mix_cmd_var_show_attributes (char *command, char **argv, int argc)
{
  int attr;
  char *attstr;
  struct varobj *var;
  ...
  if (argc != 1)
    error (_("mix_cmd_var_show_attributes: Usage: NAME."));

  /* Get varobj handle, if a valid var obj name was specified */
  var = mix_varobj_get_handle (argv[0]);
  if (var == NULL)
    error (_("mix_cmd_var_show_attributes: Variable object not found"));

  attr = mix_varobj_get_attributes (var);
...
extern int  set_unwind_on_signal (int new_val);
extern void do_set_unwind_on_signal (void *var);

enum mix_cmd_result
mix_cmd_var_evaluate_expression (char *command, char **argv, int argc)
{
  struct varobj *var;
  ...
  if (argc == 1)
  ...
  else if (argc == 2)
    {
      if (strcmp (argv[0], "-u") != 0)
        error ("mix_cmd_var_evaluate_expression: Usage: [-u] NAME.");
      else
        {
  ...
  var = mix_varobj_get_handle (expr);
  if (var == NULL)
    error (_("mix_cmd_var_evaluate_expression: Variable object not found"));

  ui_out_field_string (uiout, "value", mix_varobj_get_value (var));
...
enum mix_cmd_result
mix_cmd_var_assign (char *command, char **argv, int argc)
{
  struct varobj *var;
  char *expression;

  if (argc != 2)
    error (_("mix_cmd_var_assign: Usage: NAME EXPRESSION."));

  /* Get varobj handle, if a valid var obj name was specified */
  var = mix_varobj_get_handle (argv[0]);
  if (var == NULL)
    error (_("mix_cmd_var_assign: Variable object not found"));

  /* FIXME: define masks for attributes */
  if (!(mix_varobj_get_attributes (var) & 0x00000001))
    error (_("mix_cmd_var_assign: Variable object is not editable"));

  expression = xstrdup (argv[1]);

  if (!mix_varobj_set_value (var, expression))
    error (_("mix_cmd_var_assign: Could not assign expression to varible object"));
...
mix_prepare_tmp_out ()
{
  /* Make sure the mix_tmp_out that we use for suppressing error
     output from varobj_update is up to date. */

  if (mix_tmp_out == NULL)
    mix_tmp_out = mix_out_new (mix_version (uiout));
  else if (mix_version (mix_tmp_out) != mix_version (uiout))
    {
      ui_out_delete (mix_tmp_out);
      mix_tmp_out = mix_out_new (mix_version (uiout));
    }
}
...
enum mix_cmd_result
mix_cmd_var_update (char *command, char **argv, int argc)
{
  struct varobj *var;
  ...
  if (argc == 0)
    error (_("mix_cmd_var_update: Usage: [PRINT_VALUES] NAME [NAME...]."));

  if (argv[0][0] == '-')
    {
      print_values = mix_parse_values_option (argv[0]);
      argv++;
      argc--;
    }
 ...
 /* Check if the parameter is a "*" which means that we want to update all variables */

  if ((argc == 1) && (*argv[0] == '*') && (*(argv[0] + 1) == '\0'))
    {
      nv = mix_varobj_list (&rootlist);
      ...
      while (*cr != NULL)
        {
          mix_varobj_update_one (*cr, print_values);
          cr++;
        }
      xfree (rootlist);
      do_cleanups (cleanup);
  ...
  else
    {
      ...
      for (i = 0; i < argc; i++)
        {
          /* Get varobj handle, if a valid var obj name was specified */
          var = mix_varobj_get_handle (argv[i]);
          if (var == NULL)
            error ("mix_cmd_var_update: Variable object \"%s\" not found.", argv[i]);
          
          mix_varobj_update_one (var, print_values);
        }
      do_cleanups (cleanup);
    }
    return MIX_CMD_DONE;
...
/* Helper for mix_cmd_var_update().  */

static int
mix_varobj_update_one (struct varobj *var, enum print_values print_values)
{
  ...
  uiout = mix_tmp_out;
  nc = mix_varobj_update (&var, &changelist);
  do_cleanups (cleanup);
  ...
  if (nc == 0)
  ...
  else if (nc == -2)
    {
      ...
      ui_out_field_int (uiout, "new_num_children", mix_varobj_get_num_children(var));
      do_cleanups (cleanup);
    }
  else
    {
      ...
      var = mix_varobj_changelist_pop (changelist, &type_changed);
      ...
      while (var != NULL)
        ...
        /* APPLE LOCAL: each varobj tuple is named with VAROBJ; not anonymous */
          cleanup = make_cleanup_ui_out_tuple_begin_end (uiout, "varobj");
          ui_out_field_string (uiout, "name", mix_varobj_get_objname (var));
          if (mix_print_value_p (mix_varobj_get_gdb_type (var), print_values))
            ui_out_field_string (uiout, "value", mix_varobj_get_value (var));
          ui_out_field_string (uiout, "in_scope", "true");
          if (type_changed == VAROBJ_TYPE_UNCHANGED)
            ...
            {
              ui_out_field_string (uiout, "type_changed", "true");
              ui_out_field_string (uiout, "new_dynamic_type", mix_varobj_get_dynamic_type (var));
              ui_out_field_int (uiout, "new_num_children", mix_varobj_get_num_children(var));
            }
